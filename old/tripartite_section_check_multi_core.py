...
사용: python tripartite_section_check.py input.ts [--workers N]
필수: ffmpeg, ffprobe (PATH)
패키지: pip install -r requirements_tripartite.txt  (opencv-python, numpy)
자원: CPU만 사용 (ffmpeg 디코딩 + NumPy 비교). --workers로 코스 스캔 병렬화 가능. GPU 미사용.
"""
...
def extract_frame(path: Path, time_sec: float, width: int, height: int) -> np.ndarray | None:
    """지정 시점의 1프레임을 BGR numpy 배열로 반환. 실패 시 None."""
    cmd = [
...
def is_tripartite_frame(frame: np.ndarray, left_right_only: bool = True) -> bool:
    """
    프레임이 삼분할인지 판정. 높이 중앙부만 사용.
    left_right_only=True: 중앙 무시, 좌·우 동일성만 비교 (중앙에만 효과가 있을 때 권장).
    left_right_only=False: 좌=중앙=우 모두 비교 (기본 삼분할).
    """
...
def _coarse_worker(args: tuple) -> tuple[float, bool]:
    """멀티프로세싱용: (path_str, t, width, height, left_right_only) → (t, is_tripartite)."""
    path_str, t, width, height, left_right_only = args
    path = Path(path_str)
    frame = extract_frame(path, t, width, height)
    if frame is None:
        return (t, False)
    return (t, is_tripartite_frame(frame, left_right_only))


def _check_tripartite_at(
    path: Path, t: float, width: int, height: int, left_right_only: bool = True
) -> bool:
    """시점 t에서 1프레임 추출 후 삼분할 여부 반환."""
    frame = extract_frame(path, t, width, height)
    return frame is not None and is_tripartite_frame(frame, left_right_only)


def _binary_search_first_tripartite(
    path: Path, t_low: float, t_high: float, width: int, height: int,
    left_right_only: bool = True,
) -> float:
    """[t_low, t_high] 구간에서 삼분할이 처음 나오는 시점을 이진 탐색. (t_high에서 True인 전제)"""
    if _check_tripartite_at(path, t_low, width, height, left_right_only):
        return t_low
    while t_high - t_low > BOUNDARY_TOLERANCE:
        mid = (t_low + t_high) * 0.5
        if _check_tripartite_at(path, mid, width, height, left_right_only):
            t_high = mid
        else:
            t_low = mid
    return t_high
...
def _binary_search_last_tripartite(
    ...
    if _check_tripartite_at(path, t_high, width, height, left_right_only):
        return t_high
    while t_high - t_low > BOUNDARY_TOLERANCE:
        mid = (t_low + t_high) * 0.5
        if _check_tripartite_at(path, mid, width, height, left_right_only):
...
def find_segments(
    ...
    total_points = len(ts)
    worker_args = [(path_str, t, width, height, left_right_only) for t in ts]
    ...
    if n_workers <= 1:
        # 순차 처리
        for i, (_path_s, t, w, h, lr) in enumerate(worker_args):
            frame = extract_frame(path, t, w, h)
            if frame is None:
                coarse_results.append((t, False))
            else:
                coarse_results.append((t, is_tripartite_frame(frame, lr)))
            if (i + 1) % 30 == 0:
    ...
    while i < len(coarse_results):
        ...
        t_start = _binary_search_first_tripartite(
            path, t_prev, t_start_cand, width, height, left_right_only
        )
        t_end = _binary_search_last_tripartite(
            path, t_end_cand, min(t_next, duration), width, height, left_right_only
        )
...
def main() -> None:
    ...
    print(f"코스 스캔: 간격 {coarse}s, 병렬 프로세스 {workers}개 (CPU 사용)")
    print("-" * 50)
