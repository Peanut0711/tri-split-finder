...
def is_tripartite_frame(
    ...
    compare_mode: lr=좌·우, lc=좌·중, cr=중·우, lc_rc=좌·중·과·중·우 모두.
    use_gpu=True이고 CuPy 있으면 GPU에서 crop/MSE 수행.
    """
    if use_gpu and CUPY_AVAILABLE:
        return _is_tripartite_frame_gpu(frame, compare_mode, align_tolerance_px)
    return _is_tripartite_frame_cpu(frame, compare_mode, align_tolerance_px)
...
def get_tripartite_mse(
    ...
    반환: (is_tripartite, left_right/left_center/right_center MSE 및 필요 시 best_L).
    """
    cropped = crop_middle_y(frame)
    w = cropped.shape[1]
    base = w // 3
    ...
    if use_gpu and CUPY_AVAILABLE:
        ...
        for L in L_vals:
            ...
            mse_lr = (
                float(cp.mean((left - right) ** 2).item()) if left.shape == right.shape else float("inf")
            )
            mse_lc = float(cp.mean((lc - cc) ** 2).item())
            mse_rc = float(cp.mean((rc - cc) ** 2).item())
            score = _tripartite_optim_score(mse_lr, mse_lc, mse_rc, compare_mode)
            if score < best_score:
                best_score = score
                best_mse_lr, best_mse_lc, best_mse_rc = mse_lr, mse_lc, mse_rc
                best_L = L
        ok = _tripartite_passes(best_mse_lr, best_mse_lc, best_mse_rc, compare_mode)
        return (
            ok,
            {
                "left_right": best_mse_lr,
                "left_center": best_mse_lc,
                "right_center": best_mse_rc,
                "best_L": float(best_L),
            },
        )
...
def _export_verify_frames(
    ...
    mse_val = _primary_mse_for_display(mse_dict, compare_mode)
    ...
    if not ok:
        lr_m = mse_dict.get("left_right", 0)
        lc_m = mse_dict.get("left_center", 0)
        rc_m = mse_dict.get("right_center", 0)
        if compare_mode == "lr":
            reason = f"L vs R MSE={lr_m:.1f} (limit {MSE_THRESHOLD})."
        elif compare_mode == "lc":
            reason = f"L vs C MSE={lc_m:.1f} (limit {MSE_THRESHOLD})."
        elif compare_mode == "cr":
            reason = f"C vs R MSE={rc_m:.1f} (limit {MSE_THRESHOLD})."
        else:
            reason = f"L-C={lc_m:.1f}, R-C={rc_m:.1f} (limit {MSE_THRESHOLD})."
        cv2.putText(
    ...
    heat_a = heat_b = None
    heat_w = left.shape[1]
    if compare_mode == "lr" and left.shape == right.shape:
        heat_a, heat_b = left, right
    elif compare_mode == "lc" and left.shape == center.shape:
        heat_a, heat_b = left, center
    elif compare_mode == "cr" and center.shape == right.shape:
        heat_a, heat_b = center, right
    if heat_a is not None and heat_b is not None:
        diff_sq = (heat_a.astype(np.float64) - heat_b.astype(np.float64)) ** 2
        ...
        heatmap = cv2.resize(heatmap, (heat_w, h_min))
    ...
    if export_prefix is not None:
        lcr_path = export_dir / f"{export_prefix}_lcr_{'OK' if ok else f'NG_mse{mse_val:.0f}'}.png"
    else:
...
def verify_segment(
    ...
    while t <= end_sec:
        ...
        ok, mse_dict = get_tripartite_mse(
            frame, compare_mode, use_gpu, align_tolerance_px=align_tolerance_px
        )
        samples.append((t, ok, mse_dict))
        mse_str = ", ".join(f"{k}={v:.1f}" for k, v in mse_dict.items() if k != "best_L")
        if "best_L" in mse_dict:
            mse_str += f", best_L={mse_dict['best_L']:.0f}"
        ...
        if export_dir is not None and CV2_AVAILABLE and (export_max <= 0 or export_count < export_max):
            if not export_only_x or not ok:
                if _export_verify_frames(frame, t, ok, mse_dict, compare_mode, export_dir):
                    export_count += 1
        t += interval_sec
...
def _coarse_worker(args: tuple) -> tuple[float, bool]:
    ...
    is_tri = is_tripartite_frame(
        frame, compare_mode, use_gpu, align_tolerance_px=align_tol_px
    )
                ...
                if frame is None:
                    ...
                    if cache_dir is not None:
                        try:
                            ...
                            pass
                    is_tri = is_tripartite_frame(frame, cm, ug, align_tolerance_px=align_tol)
...
def main() -> None:
    ...
    try:
        ...
        if getattr(args, "debug_frame", None) is not None:
            ...
            ok, mse_dict = get_tripartite_mse(
                frame, compare_mode, use_gpu, align_tolerance_px=align_tol
            )
            ...
            verdict_kr = "삼분할 감지 성공" if ok else "삼분할 감지 실패"
            lr_m = mse_dict.get("left_right", 0)
            lc_m = mse_dict.get("left_center", 0)
            rc_m = mse_dict.get("right_center", 0)
            if compare_mode == "lr":
                reason = f"좌·우 MSE={lr_m:.1f} (기준 {MSE_THRESHOLD})" + (" 이하 → 동일로 판정" if ok else " 초과 → 불일치")
            elif compare_mode == "lc":
                reason = f"좌·중 MSE={lc_m:.1f} (기준 {MSE_THRESHOLD})" + (" 이하 → 동일로 판정" if ok else " 초과 → 불일치")
            elif compare_mode == "cr":
                reason = f"중·우 MSE={rc_m:.1f} (기준 {MSE_THRESHOLD})" + (" 이하 → 동일로 판정" if ok else " 초과 → 불일치")
            else:
                reason = f"좌-중 MSE={lc_m:.1f}, 중-우 MSE={rc_m:.1f} (기준 {MSE_THRESHOLD})" + (" 모두 이하" if ok else " 중 하나 이상 초과")
