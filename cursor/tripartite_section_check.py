...
def _export_verify_frames(
    ...
    if not ok:
        if left_right_only:
            reason = f"Left vs right differ; MSE={mse_val:.1f} > threshold ({MSE_THRESHOLD})."
        else:
            lc = mse_dict.get("left_center", 0)
            rc = mse_dict.get("right_center", 0)
            reason = f"L-C or R-C MSE exceeds threshold. L-C={lc:.1f}, R-C={rc:.1f} (limit {MSE_THRESHOLD})."
        cv2.putText(
...
def main() -> None:
    ...
    try:
        ...
        if getattr(args, "debug_frame", None) is not None:
            ...
            ok, mse_dict = get_tripartite_mse(
                frame, left_right_only, use_gpu, align_tolerance_px=align_tol
            )
            ...
            if CV2_AVAILABLE:
                _export_verify_frames(frame, time_sec, ok, mse_dict, left_right_only, debug_dir, export_prefix=debug_export_prefix)
            verdict_kr = "삼분할 감지 성공" if ok else "삼분할 감지 실패"
            mse_val = next((v for k, v in mse_dict.items() if k != "best_L"), 0)
            if left_right_only:
                reason = f"좌·우 MSE={mse_val:.1f} (기준 {MSE_THRESHOLD})" + (" 이하 → 동일로 판정" if ok else " 초과 → 불일치")
            else:
                lc = mse_dict.get("left_center", 0)
                rc = mse_dict.get("right_center", 0)
                reason = f"좌-중앙 MSE={lc:.1f}, 우-중앙 MSE={rc:.1f} (기준 {MSE_THRESHOLD})" + (" 모두 이하" if ok else " 중 하나 이상 초과")
