...
def test_tripartite_orb(image_path, tol=3, min_matches=15):
    """
    ORB 특징점 매칭을 이용해 삼분할 영상 여부를 판별합니다.
    
    :param image_path: 테스트할 이미지 경로
    :param tol: 좌우 매칭 시 허용할 픽셀 위치 오차 (1/4 해상도 기준, 기본 3픽셀)
    :param min_matches: 좌/우 각각 최소 몇 개의 특징점이 일치해야 통과시킬지 기준 (1/4 해상도 기준, 기본 15개)
    """
    # 1. 이미지 로드
    ...
    # 5. 특징점 매칭 (해밍 거리 사용, 상호 일치하는 것만 허용)
    ...
    # 화면에 결과 띄우기
    cv2.imshow("ORB Match Result (1/4 scale)", debug_img)
...
test_tripartite_orb(
    ...
    tol=3,            # [수정됨] 1/4 해상도 기준 픽셀 오차 허용치
    min_matches=15    # [수정됨] 1/4 해상도에서의 최소 매칭 개수
)