...
Y_CROP_RATIO_START = 0.55  # 높이 상 35% 지점부터 샘플 (상단 팝업 제외)
Y_CROP_RATIO_END = 0.75  # 높이 상 65% 지점까지 샘플 (중앙부만 사용)
...
def extract_frames_batch(
    ...
    """구간 [start_t, end_t]를 한 번의 ffmpeg로 디코딩해 interval_sec 간격 프레임을 반환.
    코스 스캔 시 시점당 ffmpeg 한 번씩 호출하는 대신, 워커당 한 번만 호출해 시간을 크게 단축."""
    if interval_sec <= 0 or end_t <= start_t:
        return []
    out_w = scale_width if scale_width is not None else width
    out_h = round(height * out_w / width) if scale_width is not None else height
    n_frames = max(1, int(round((end_t - start_t) / interval_sec)) + 1)
    duration = end_t - start_t + 1.0
    vf_parts = [f"fps=1/{interval_sec}"]
    if scale_width is not None:
        vf_parts.append(f"scale={out_w}:-1")
    vf = ",".join(vf_parts)
    base = ["-ss", str(start_t), "-i", str(path), "-t", str(duration), "-vf", vf]
    tail = ["-f", "rawvideo", "-pix_fmt", "bgr24", "-an", "-sn", "pipe:1"]
    cmd = ["ffmpeg", "-y"]
    if use_gpu:
        cmd.extend(["-hwaccel", "cuda"])
    cmd.extend(base + tail)
    try:
        out = subprocess.run(cmd, capture_output=True, timeout=max(60, int(duration / 10) + 30))
        if out.returncode != 0 or not out.stdout:
            if use_gpu:
                cmd_cpu = ["ffmpeg", "-y"] + base + tail
                out = subprocess.run(cmd_cpu, capture_output=True, timeout=max(60, int(duration / 10) + 30))
            if out.returncode != 0 or not out.stdout:
                return []
        raw = np.frombuffer(out.stdout, dtype=np.uint8)
        frame_size = out_h * out_w * 3
        if len(raw) < frame_size * n_frames:
            n_frames = len(raw) // frame_size
        if n_frames <= 0:
            return []
        frames = raw[: n_frames * frame_size].reshape((n_frames, out_h, out_w, 3))
        return [(start_t + i * interval_sec, frames[i]) for i in range(n_frames)]
    except (subprocess.TimeoutExpired, ValueError, FileNotFoundError):
        return []
...
def _coarse_worker_batch(args: tuple) -> list[tuple[float, bool]]:
    """멀티프로세싱용: 한 구간의 시점 리스트를 받아, 한 번의 ffmpeg 배치 추출 후 각각 삼분할 판정.
    (path_str, ts_list, interval_sec, width, height, left_right_only, use_gpu, scale_width, cache_dir_str, align_tolerance_px) → [(t, bool), ...]"""
    path_str, ts_list, interval_sec, width, height, left_right_only, use_gpu, scale_width, cache_dir_str, align_tol_px = args
    path = Path(path_str)
    if not ts_list:
        return []
    align_tol = int(align_tol_px) if align_tol_px is not None else None
    start_t, end_t = ts_list[0], ts_list[-1]
    # 한 번의 ffmpeg로 구간 내 interval_sec 간격 프레임 일괄 추출
    batch_results = extract_frames_batch(
        path, start_t, end_t, interval_sec, width, height, use_gpu, scale_width=scale_width
    )
    frame_by_t = {t: f for t, f in batch_results}
    out: list[tuple[float, bool]] = []
    for t in ts_list:
        frame = frame_by_t.get(t)
        if frame is None:
            frame = extract_frame(path, t, width, height, use_gpu, scale_width=scale_width)
        if frame is None:
            out.append((t, False))
        ...
        if cache_dir_str:
            try:
                np.save(Path(cache_dir_str) / f"frame_{t:.3f}.npy", frame)
            except (OSError, ValueError):
                pass
        out.append((t, is_tripartite_frame(frame, left_right_only, use_gpu, align_tolerance_px=align_tol)))
    return out
...
def find_segments(
    ...
    try:
        ...
        if n_workers <= 1:
            ...
            # 배치 모드: 시점을 워커 수만큼 구간으로 나누고, 구간당 한 번의 ffmpeg로 연속 디코딩 (시점당 1회 호출 → 워커당 1회로 축소)
            chunk_size = max(1, (total_points + n_workers - 1) // n_workers)
            chunks = [ts[i : i + chunk_size] for i in range(0, total_points, chunk_size)]
            n_chunks = len(chunks)
            print(
                f"  코스 스캔(배치): {n_chunks}개 구간, 총 {total_points}개 시점. 구간당 1회 ffmpeg 실행.",
                flush=True,
            )
            print(
                f"  → 첫 진행 로그는 첫 구간이 끝난 뒤 출력됩니다. 구간당 영상 길이에 따라 수 분 걸릴 수 있으니 대기해 주세요.",
                flush=True,
            )
            batch_args = [
                (path_str, chunk, coarse_interval_sec, width, height, left_right_only, use_gpu, scale_w, cache_dir_str, align_tolerance_px)
                for chunk in chunks
            ]
            done = 0
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                for chunk_idx, chunk_results in enumerate(executor.map(_coarse_worker_batch, batch_args, chunksize=1), start=1):
                    coarse_results.extend(chunk_results)
                    done += len(chunk_results)
                    elapsed = time.perf_counter() - scan_start
                    elapsed_str = f"{int(elapsed // 60)}분 {int(elapsed % 60)}초" if elapsed >= 60 else f"{int(elapsed)}초"
                    pct = (done / total_points * 100) if total_points > 0 else 0
                    print(
                        f"  진행: 구간 {chunk_idx}/{n_chunks} 완료  ({done}/{total_points}, {pct:.1f}%)  |  실행 경과: {elapsed_str}",
                        flush=True,
                    )
