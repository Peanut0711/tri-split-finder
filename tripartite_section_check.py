...
def _boundary_worker(args: tuple) -> tuple[float, float] | None:
    ...
    (path_str, t_prev, t_start_cand, t_end_cand, t_next, duration, width, height, left_right_only, use_gpu)
    → (t_start, t_end) 또는 None.
    """
    (
        path_str,
        t_prev,
...
def find_segments(
    ...
    candidates: list[tuple[str, float, float, float, float, float, int, int, bool, bool]] = []
    i = 0
    ...
    # 3) 경계 정밀화 — 구간 간 병렬(CPU 멀티코어) 또는 순차 (발견 + 삼분할 구간 찾기)
